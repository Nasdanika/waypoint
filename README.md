...
- **Tibco BusinessWorks 5** process tokens, which carry payload along a process and support rollback / retry through transaction groups.
...
The original Tibco BW 5 motivation. The token graph itself is append-only; rollback is achieved by:
...
3. For `StateToken`, `pred.getState()` is the starting state for the retry — exactly what BW 5 does with process variables in a transaction group.
